...
export function normalizeMissingPerson(item, index = 0) {
  ...
  return {
    ...
    photoUrl: normalizePhotoUrl(firstValue(item, ["photoUrl", "tknphotoFile", "photo", "imageUrl"])),
...
export function extractRegion(locationText) {
  ...
  const cityCountyMap = new Map([
    ["순천", "전라남도"], ["순천시", "전라남도"], ["강진", "전라남도"], ["강진군", "전라남도"], ["강진군읍", "전라남도"],
    ["경주", "경상북도"], ["경주시", "경상북도"],
    ["강화", "인천광역시"], ["강화군", "인천광역시"], ["강화읍", "인천광역시"],
    ["성남", "경기도"], ["성남시", "경기도"], ["성남수정", "경기도"], ["수정구", "경기도"]
  ]);
