...
const KNU_CENTER = { lat: 35.8908, lng: 128.6111 }; // 대구 경북대학교(대략)
      ...
      if (hasNearby) {
        ...
        visible.forEach((person) => {
          ...
          if (latDiff <= 0.25 && lngDiff <= 0.25) {
            nearbyBounds.extend(new window.kakao.maps.LatLng(person.lat, person.lng));
          }
        });
...
function App() {
  ...
  const loadAlerts = async () => {
    ...
    setError("");
    try {
      const data = await fetchJson(`/api/missing/alerts?rowSize=${ALERT_ROW_SIZE}`);
    ...
    <section className="map-layout" aria-labelledby="map-title">
      <div className="map-area">
        <div className="toolbar">
          ...
          <div className="toolbar-actions">
            <label className="time-filter">
              기간
              <select value={timeFilter} onChange={(event) => onChangeTimeFilter(event.target.value)}>
        ...
        <div className="marker-legend">
          <span><i className="dot week" /> 최근 1주</span>
          <span><i className="dot month" /> 최근 1개월</span>
          <span><i className="dot older" /> 1개월 초과</span>
          <span><i className="dot unknown" /> 날짜 미상</span>
        </div>
        ...
        <div className="map-canvas-wrap">
          ...
          {selected && isDetailOpen && (
            <div className="detail-modal-backdrop" role="presentation" onClick={onCloseDetail}>
              <article
              ...
              >
                <button className="detail-modal-close" type="button" onClick={onCloseDetail} aria-label="상세 닫기">
                  ×
                </button>
